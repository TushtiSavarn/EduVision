...
def teacher_dashboard(request):
    ...
    cs_id = request.POST.get('course_subject_id') or request.GET.get('cs_id')
    course_subject = CourseSubject.objects.filter(id=cs_id).first()

    # 🔥 check update mode
    update_mode = request.GET.get('update') == 'true'

    # 🔥 check if data exists
    data_exists = False
    if course_subject and not update_mode:
        data_exists = Marks.objects.filter(course_subject=course_subject).exists()

    students = Student.objects.filter(course=course_subject.course) if course_subject else []
    ...
    if request.method == "POST" and course_subject:
        show_result = True
